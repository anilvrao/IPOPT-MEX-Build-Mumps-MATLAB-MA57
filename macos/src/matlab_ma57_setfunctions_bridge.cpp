...
MatlabMa57& lib()
{
  ...
  if (!L->handle) {
    ...
    if (mexCallMATLAB(1, lhs, 0, nullptr, "matlabroot") != 0 || lhs[0] == nullptr) {
      std::fprintf(stderr, "IPOPT MA57 bridge: unable to query matlabroot\n");
      std::abort();
    }
    ...
    if (root == nullptr) {
      std::fprintf(stderr, "IPOPT MA57 bridge: unable to convert matlabroot to a path\n");
      std::abort();
    }
    ...
    if (!L->handle) {
      std::fprintf(stderr, "IPOPT MA57 bridge: unable to load %s: %s\n", path.c_str(), dlerror());
      std::abort();
    }
    ...
    if (!L->ma57ad || !L->ma57bd || !L->ma57cd || !L->ma57ed || !L->ma57id) {
      std::fprintf(stderr, "IPOPT MA57 bridge: unable to resolve required MA57 symbols\n");
      std::abort();
    }
  }
  return *L;
...
void to32(ipma57int* x, const std::vector<mwma57int>& y)
{
  for (std::size_t k = 0; k < y.size(); ++k) {
    if (y[k] > static_cast<mwma57int>(std::numeric_limits<ipma57int>::max()) ||
      ...
      std::fprintf(stderr, "IPOPT MA57 bridge: MATLAB MA57 returned an integer outside Ipopt range\n");
      std::abort();
    }
