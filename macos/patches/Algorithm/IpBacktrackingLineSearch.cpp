   ...
   if( fallback_activated_ )
   {
      ...
      else
      {
         Jnlst().Printf(J_ERROR, J_LINE_SEARCH,
                        "Step computation failed in emergency mode, and no restoration phase or other fallback is available.\n");
         skipped_line_search_ = true;
         IpData().Set_info_alpha_primal_char('E');
         fallback_activated_ = false;
         return;
      }
   ...
   if( !accept )
   {
      ...
      else
      {
         ...
         if( !accept )
         {
            ...
            {
               skipped_line_search_ = true;
               IpData().Set_info_alpha_primal_char('A');
               return;
            }
            ...
            {
               Jnlst().Printf(J_ERROR, J_LINE_SEARCH,
                              "No restoration phase is available to this backtracking line search object.\n");
               skipped_line_search_ = true;
               IpData().Set_info_alpha_primal_char('E');
               return;
            }
            ...
            catch( IpoptException& exc )
            {
               ...
               Jnlst().Printf(J_WARNING, J_LINE_SEARCH,
                              "Restoration phase reported failure; returning control to the main algorithm without throwing through the MEX boundary.\n");
               skipped_line_search_ = true;
               IpData().Set_info_alpha_primal_char('E');
               return;
            }
            ...
            {
               Jnlst().Printf(J_WARNING, J_LINE_SEARCH,
                              "Restoration phase threw a C++ exception: %s\n",
                              exc.what());
               skipped_line_search_ = true;
               IpData().Set_info_alpha_primal_char('E');
               return;
            ...
            {
               Jnlst().Printf(J_WARNING, J_LINE_SEARCH,
                              "Restoration phase threw an unknown exception.\n");
               skipped_line_search_ = true;
               IpData().Set_info_alpha_primal_char('E');
               return;
            }
            if( !accept )
            {
               ...
               {
                  Jnlst().Printf(J_WARNING, J_LINE_SEARCH,
                                 "Restoration phase failed and no acceptable point could be restored.\n");
                  skipped_line_search_ = true;
                  IpData().Set_info_alpha_primal_char('E');
                  return;
               }
