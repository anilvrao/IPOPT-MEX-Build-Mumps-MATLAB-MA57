...
#   - Ipopt 3.14.19 rebuilt against MUMPS 5.9.1 headers/libraries
#   - MUMPS 5.9.1 from official source via current Coin-OR ThirdParty-Mumps
#   - existing source-built OpenBLAS archive
#   - no SPRAL, METIS, hwloc, GNU OpenMP, or GNU C++ runtime
...
BASE="${IPOPT_MEX_BASE:-$ROOT/ipopt_mumps_source_mac_build}"
...
MEX_DRIVER="${MEX_DRIVER:-$MATLAB_ROOT/bin/mex}"
INTERFACE_SOURCE="${IPOPT_MEX_INTERFACE_SOURCE:-$PACKAGE_ROOT/src/IpoptInterfaceCommon_safe.cc}"
...
GCC_ROOT="${GCC_ROOT:-/opt/homebrew/opt/gcc/lib/gcc/current}"
GFORTRAN="${GFORTRAN:-/opt/homebrew/bin/gfortran-16}"
GFORTRAN_LIB="$GCC_ROOT/libgfortran.a"
QUADMATH_LIB="$GCC_ROOT/libquadmath.a"
ATOMIC_LIB="$GCC_ROOT/libatomic.a"
GCC_LIB="${GCC_LIB:-$(find "$GCC_ROOT/gcc" -name libgcc.a -print -quit)}"
...
for required in \
  ...
  "$MUMPS_FILELIST" \
  "$PACKAGE_ROOT/patches/Algorithm/IpBacktrackingLineSearch.cpp" \
  "$PACKAGE_ROOT/patches/Algorithm/IpIpoptAlg.cpp" \
  "$PACKAGE_ROOT/patches/Algorithm/IpRestoMinC_1Nrm.cpp" \
  "$PACKAGE_ROOT/src/ipopt_setfunctions.cc" \
  "$INTERFACE_SOURCE" \
  "$PACKAGE_ROOT/src/IpoptInterfaceCommon.hh" \
  "$PACKAGE_ROOT/src/matlab_ma57_setfunctions_bridge.cpp" \
  "$OPENBLAS_LIB" \
  "$GFORTRAN" \
  "$GFORTRAN_LIB" \
  "$QUADMATH_LIB" \
...
cp "$PACKAGE_ROOT/patches/Algorithm/IpBacktrackingLineSearch.cpp" \
  "$IPOPT_SRC/src/Algorithm/IpBacktrackingLineSearch.cpp"
cp "$PACKAGE_ROOT/patches/Algorithm/IpIpoptAlg.cpp" \
  "$IPOPT_SRC/src/Algorithm/IpIpoptAlg.cpp"
cp "$PACKAGE_ROOT/patches/Algorithm/IpRestoMinC_1Nrm.cpp" \
...
echo "Configuring and building Ipopt against MUMPS 5.9.1..."
...
cp "$PACKAGE_ROOT/src/ipopt_setfunctions.cc" "$MEX_BUILD/ipopt_setfunctions.cc"
cp "$INTERFACE_SOURCE" "$MEX_BUILD/IpoptInterfaceCommon_safe.cc"
cp "$PACKAGE_ROOT/src/IpoptInterfaceCommon.hh" "$MEX_BUILD/IpoptInterfaceCommon.hh"
cp "$PACKAGE_ROOT/src/matlab_ma57_setfunctions_bridge.cpp" "$MEX_BUILD/matlab_ma57_setfunctions_bridge.cpp"
...
args = [{'-largeArrayDims', ['-I', mexBuild], '-Isrc', ...
  ...
  {'LDFLAGS=\$LDFLAGS -Wl,-ld_classic -Wl,-dead_strip -ldl', ...
  'LINKLIBS=\$LINKLIBS -Wl,-force_load,$FLAT_LIB -Wl,-filelist,$MUMPS_FILELIST $OPENBLAS_LIB $GFORTRAN_LIB $QUADMATH_LIB $GCC_LIB $ATOMIC_LIB', ...
...
echo "Compiling MATLAB MEX with the standalone driver..."
pushd "$MEXIPOPT" >/dev/null
"$MEX_DRIVER" -largeArrayDims \
  "-I$MEX_BUILD" -Isrc \
  "-I$IPOPT_INSTALL/include/coin-or" \
  "-I$IPOPT_SRC/src/Algorithm/LinearSolvers" \
  -DOS_MAC -output "$MEX_BIN/ipopt" \
  "$MEX_BUILD/ipopt_setfunctions.cc" \
  "$MEX_BUILD/IpoptInterfaceCommon_safe.cc" \
  "$MEX_BUILD/matlab_ma57_setfunctions_bridge.cpp" \
  "$MEX_BUILD/ma57_link_stubs.cpp" \
  'LDFLAGS=$LDFLAGS -Wl,-ld_classic -Wl,-dead_strip -ldl' \
  "LINKLIBS=\$LINKLIBS -Wl,-force_load,$FLAT_LIB -Wl,-filelist,$MUMPS_FILELIST $OPENBLAS_LIB $GFORTRAN_LIB $QUADMATH_LIB $GCC_LIB $ATOMIC_LIB" \
  'CXXFLAGS=$CXXFLAGS -Wall -O2 -DFUNNY_MA57_FINT -fno-c++-static-destructors'
popd >/dev/null
