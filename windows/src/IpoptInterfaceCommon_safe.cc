  ...
  */
  ...
  IpoptOptions::setStringOption( char const * label, mxArray const * ptr ) {
    ...
    if (!success && std::strcmp(label, "linear_solver") == 0) {
      ...
      mxFree(value);
      IPOPT_DO_ERROR("Invalid IPOPT option: linear_solver. Supported values are 'mumps' and 'ma57'.");
    }
    IPOPT_ASSERT(
  ...
  */
  ...
  MatlabProgram::eval_f(
    ...
    try {
      ...
      if ( !std::isfinite(f) ) f = 1e20;
    } catch ( std::exception const & e ) {
      mexPrintf("Warning: IPOPT objective callback failed at a trial point: %s\n", e.what());
      mexEvalString("drawnow;");
      f = 1e20;
    } catch ( ... ) {
      mexPrintf("Warning: IPOPT objective callback failed at a trial point.\n");
      mexEvalString("drawnow;");
      f = 1e20;
    }
  ...
  MatlabProgram::eval_grad_f(
    ...
    try {
      ...
      mexPrintf("Warning: IPOPT gradient callback failed at a trial point: %s\n", e.what());
      mexEvalString("drawnow;");
      std::fill_n( grad, n, 0.0 );
    } catch ( ... ) {
      mexPrintf("Warning: IPOPT gradient callback failed at a trial point.\n");
      mexEvalString("drawnow;");
      std::fill_n( grad, n, 0.0 );
    }
  ...
  MatlabProgram::eval_g(
    ...
    if ( m > 0 ) {
      ...
      try {
        ...
        for ( Index i = 0; i < m; ++i ) {
          if ( !std::isfinite(g[i]) ) g[i] = 1e20;
        }
      } catch ( std::exception const & e ) {
        mexPrintf("Warning: IPOPT constraint callback failed at a trial point: %s\n", e.what());
        mexEvalString("drawnow;");
        std::fill_n( g, m, 1e20 );
      } catch ( ... ) {
        mexPrintf("Warning: IPOPT constraint callback failed at a trial point.\n");
        mexEvalString("drawnow;");
        std::fill_n( g, m, 1e20 );
      }
    }
    ...
    if ( numConstraints > 0 ) {
      ...
      if ( Jx == nullptr ) {
        ...
        try {
          ...
          for ( Index i = 0; i < Jx_nnz; ++i ) {
            if ( !std::isfinite(Jx[i]) ) Jx[i] = 0.0;
          }
        } catch ( std::exception const & e ) {
          mexPrintf("Warning: IPOPT Jacobian callback failed at a trial point: %s\n", e.what());
          mexEvalString("drawnow;");
          std::fill_n( Jx, Jx_nnz, 0.0 );
        } catch ( ... ) {
          mexPrintf("Warning: IPOPT Jacobian callback failed at a trial point.\n");
          mexEvalString("drawnow;");
          std::fill_n( Jx, Jx_nnz, 0.0 );
        }
      }
  ...
  MatlabProgram::eval_h(
    ...
    if ( Hx == nullptr ) {
      ...
      try {
        ...
        for ( Index i = 0; i < Hx_nnz; ++i ) {
          if ( !std::isfinite(Hx[i]) ) Hx[i] = 0.0;
        }
      } catch ( std::exception const & e ) {
        mexPrintf("Warning: IPOPT Hessian callback failed at a trial point: %s\n", e.what());
        mexEvalString("drawnow;");
        std::fill_n( Hx, Hx_nnz, 0.0 );
      } catch ( ... ) {
        mexPrintf("Warning: IPOPT Hessian callback failed at a trial point.\n");
        mexEvalString("drawnow;");
        std::fill_n( Hx, Hx_nnz, 0.0 );
      }
    }
