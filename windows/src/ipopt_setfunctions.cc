...
{
  if (options == nullptr || !mxIsStruct(options)) return false;

  mxArray const* ipoptOptions = mxGetField(options, 0, "ipopt");
  if (ipoptOptions == nullptr || !mxIsStruct(ipoptOptions)) return false;

  mxArray const* linearSolver = mxGetField(ipoptOptions, 0, "linear_solver");
  if (linearSolver == nullptr || !mxIsChar(linearSolver)) return false;

  char* solver = mxArrayToString(linearSolver);
  if (solver == nullptr) return false;

  bool isMa57 = (std::strcmp(solver, "ma57") == 0 ||
                 std::strcmp(solver, "MA57") == 0);
  mxFree(solver);
  return isMa57;
}

static void ipoptMexPrintInvalidLinearSolverBlock()
{
  mexPrintf("\n");
  mexPrintf("+------------------------------------------------------------+\n");
...
static bool ipoptMexOptionsHaveInvalidLinearSolver(mxArray const* options)
{
  if (options == nullptr || !mxIsStruct(options)) return false;
  ...
  bool isValid = (std::strcmp(solver, "mumps") == 0 ||
                  std::strcmp(solver, "MUMPS") == 0 ||
                  std::strcmp(solver, "ma57") == 0 ||
                  std::strcmp(solver, "MA57") == 0);
  mxFree(solver);
  return !isValid;
...
static void ipoptMexReturnInvalidLinearSolver(mxArray* plhs[])
{
  static char const* fields[] = {"status", "message"};

  ipoptMexPrintInvalidLinearSolverBlock();

  plhs[0] = mxCreateDoubleMatrix(0, 0, mxREAL);
  plhs[1] = mxCreateStructMatrix(1, 1, 2, fields);
  mxSetField(plhs[1], 0, "status", mxCreateDoubleScalar(-999));
  mxSetField(plhs[1], 0, "message",
    mxCreateString("Invalid IPOPT option: linear_solver. Supported values are 'mumps' and 'ma57'."));
}
...
mexFunction(
  ...
  try {
    ...
    if (nrhs == 3 && ipoptMexOptionsHaveInvalidLinearSolver(prhs[2])) {
      ipoptMexReturnInvalidLinearSolver(plhs);
      return;
    }
