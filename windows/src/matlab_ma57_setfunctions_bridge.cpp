...
MatlabMa57& lib()
{
  ...
  if (!L->handle) {
    ...
    std::string path = std::string(root) + "\\bin\\win64\\libmwma57.dll";
    mxFree(root);
    ...
    auto sym = [&](const char* plain, const char* underscored) -> FARPROC {
      FARPROC p = GetProcAddress(L->handle, plain);
      return p ? p : GetProcAddress(L->handle, underscored);
    };
    L->ma57ad = reinterpret_cast<mw_ma57ad_f>(sym("ma57ad", "ma57ad_"));
    L->ma57bd = reinterpret_cast<mw_ma57bd_f>(sym("ma57bd", "ma57bd_"));
    L->ma57cd = reinterpret_cast<mw_ma57cd_f>(sym("ma57cd", "ma57cd_"));
