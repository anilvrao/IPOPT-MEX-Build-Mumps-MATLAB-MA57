...
void IpoptAlgorithm::ComputeAcceptableTrialPoint()
{
   ...
   catch( IpoptException& exc )
   {
      exc.ReportException(Jnlst(), J_MOREDETAILED);
      Jnlst().Printf(J_ERROR, J_MAIN,
                     "Line search failed while computing an acceptable trial point.\n");
      return;
   }
   catch( std::exception& exc )
   {
      Jnlst().Printf(J_ERROR, J_MAIN,
                     "Line search threw a C++ exception while computing an acceptable trial point: %s\n",
                     exc.what());
      return;
   }
   catch( ... )
   {
      Jnlst().Printf(J_ERROR, J_MAIN,
                     "Line search threw an unknown exception while computing an acceptable trial point.\n");
      return;
   }
}
