...
MatlabMa57& lib()
{
  ...
  if (!L->handle) {
    ...
    if (mexCallMATLAB(1, lhs, 0, nullptr, "matlabroot") != 0 || lhs[0] == nullptr) {
      fatal("unable to query matlabroot");
    }

    char* root = mxArrayToString(lhs[0]);
    mxDestroyArray(lhs[0]);
    if (root == nullptr) {
      fatal("unable to convert matlabroot to a path");
    }

    std::string path = std::string(root) + "/bin/glnxa64/libmwma57.so";
    mxFree(root);
    ...
    if (!L->ma57ad || !L->ma57bd || !L->ma57cd || !L->ma57ed || !L->ma57id) {
      fatal("unable to resolve required MA57 symbols");
    }
  }
  return *L;
}
...
void to32(ipma57int* x, const std::vector<mwma57int>& y)
{
  for (std::size_t k = 0; k < y.size(); ++k) {
    if (y[k] > static_cast<mwma57int>(std::numeric_limits<ipma57int>::max()) ||
      ...
      fatal("MATLAB MA57 returned an integer outside Ipopt range");
    }
                  ...
                  ipma57int* newifc, ipma57int* linew, ipma57int* info)
{
  ...
  if (*ic == 0) {
    ...
    L.ma57ed(&n64, &ic64, keep64.data(), fact, &lfact64, newfac, &lnew64,
             ifact64.data(), &lifact64, &newifcDummy, &linew64, info64.data());
    if (newifc) {
